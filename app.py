...
def main():
    ...
    st.title("📊 广告数据清洗与降维合并 (Auto-Merge V4.2 + V20.10)")

    st.markdown("""
    **功能说明：**
    1. **ETL阶段 (Auto-Merge V4.2)**：完全复刻脚本1逻辑，包括字段映射、数值清洗、以及**素材/落地页 Top10 截断**。
    2. **报告阶段 (V20.10)**：复刻脚本2逻辑，生成架构诊断表、Word 报告及 Gemini 分析用 JSON。
    """)
...
# # PART 1: 配置区域
# # ==========================================
# # 这是一个全局的“字典翻译本”，决定了所有文件中列名叫什么
# REPORT_MAPPING = {
#     # 核心指标
#     "spend": "花费 ($)",
#     "roas": "ROAS",
#     "purchases": "购买次数",
#     "purchase_value": "购买总价值 ($)",
#     "cpa": "CPA (单次成效费用 $)",
#     "ctr": "CTR (点击率 %)",
#     "cpm": "CPM (千次展示费 $)",
#     "cpc": "CPC (单次点击费 $)",
...
# 
#     # 漏斗指标
#     "landing_page_views": "落地页访问量",
#     "add_to_cart": "加购次数",
#     "initiate_checkout": "结账发起数 (IC)",
#     "rate_click_to_lp": "点击→落地页转化率 (%)",
#     "rate_lp_to_atc": "落地页→加购转化率 (%)",
#     "rate_atc_to_ic": "加购→IC转化率 (%)",
#     "rate_ic_to_pur": "IC→购买转化率 (%)",
#     "cvr_purchase": "CVR (全站转化率 %)",
#     "cvr_lp_to_pur": "LP→购买转化率 (%)",
# 
#     # 维度字段
#     "date_range": "日期/时段",
#     "dimension_item": "维度名称",
...
#     "adset_name": "广告组名称",
#     "adset_id": "广告组ID",
#     "custom_audience_settings": "自定义受众源",
#     "converting_keywords": "高潜兴趣词",
#     "audience_type": "受众策略",
#     "country": "国家",
#     "converting_countries": "主要成效国家",
#     "converting_genders": "主要成效性别",
#     "converting_ages": "主要成效年龄",
#     "age_group": "年龄段",
#     "gender": "性别",
#     "creative_name": "素材名称",
#     "placement": "版位",
#     "landing_page_url": "页面 URL",
# 
#     # 诊断字段
#     "mom_change": "环比波动",
#     "anomaly_metric_name": "异常项",
#     "Label": "数据标签",
#     "metric": "指标名称",
#     "current_value": "当前数值",
#     "benchmark_value": "大盘基准",
#     "diff": "差值",
#     "status": "状态评价",
#     "Module": "模块",
#     "Stats": "统计数据",
...
#     if any(x in k for x in ['purchases', 'cart', 'click', '次数', '单量', '展现', '访问量', '发起数']): return f"{val:,.0f}"
#     return f"{val}"
# 
# def extract_benchmark_values(df_bench):
#     targets = {'roas': (['roas'], True), 'cpm': (['cpm'], False), 'ctr': (['ctr'], True), 'cpc': (['cpc'], False), 'cpa': (['cpa_purchase', 'cpa'], False)}
...
#             else:
#                 cell.text = str(val)
#                 if "结论" in str(df.columns[j]) or "评价" in str(df.columns[j]):
#                     if "✅" in str(val): cell.paragraphs[0].runs[0].font.color.rgb = RGBColor(0, 128, 0)
#                     if "⚠️" in str(val): cell.paragraphs[0].runs[0].font.color.rgb = RGBColor(255, 0, 0)
# 
#             for p in cell.paragraphs:
...
#                 add_df_to_word(self.doc, apply_report_labels(df_f_display), "1. 数据大盘总览", level=1)
# 
#                 # --- Benchmark ---
#                 bench_json = []
#                 bench_word_data = []
...
#                     if curr_val != 0:
#                         if higher_is_better:
#                             if diff > 0: status = "better"; conclusion_word = "✅ 优于大盘"
#                             elif diff < 0: status = "worse"; conclusion_word = "⚠️ 低于大盘"
#                             else: status = "flat"; conclusion_word = "持平"
#                         else:
#                             if diff < 0: status = "better"; conclusion_word = "✅ 优于大盘"
#                             elif diff > 0: status = "worse"; conclusion_word = "⚠️ 高于大盘"
#                             else: status = "flat"; conclusion_word = "持平"
# 
#                     bench_json.append({
#                         "metric": metric_key,
...
#                 # Word 里这里已经手动映射过了，所以这里用 apply_report_labels 处理剩下的
#                 add_df_to_word(self.doc, apply_report_labels(pd.DataFrame(bench_word_data)), "2. 行业 Benchmark 对比", level=1)
# 
#         # --- 3. 受众组 ---
#         self.doc.add_heading("3. 受众组分析", level=1)
...
# 
#         audience_configs = [
#             ("3.1 国家分析", ["国家", "Country"], True, "国家", "country_breakdown"),
#             ("3.2 性别分析", ["性别", "Gender"], False, "性别", "gender_breakdown"),
#             ("3.3 年龄分析", ["年龄", "Age"], False, "年龄段", "age_breakdown"),
#             ("3.4 受众组分析表", ["受众", "Audience"], True, "受众组名称", "adset_breakdown"),
#         ]
# 
#         if "Master_Breakdown" in self.merged_dfs:
#             df_bd = self.merged_dfs["Master_Breakdown"]
...
#             df_cr = self.merged_dfs["Master_Creative"]
#             configs = [
#                 ("4. 素材分析", ["素材", "Creative"], "素材名称", "creative_analysis"),
#                 ("6. 落地页分析", ["落地页", "Landing"], "落地页 URL", "landing_page_analysis")
#             ]
...
#                  }
# 
#                  for d, sub_title in [(df_top5, "5.1 版位花费 TOP 5"), (df_pot, "5.2 版位高潜力")]:
#                      if not d.empty:
#                          d_disp = d.copy()
#                          for c in d_disp.columns:
#                              if c == 'dimension_item': continue
#                              d_disp[c] = d_disp[c].apply(lambda x: format_cell_for_display(c, x))
#                          add_df_to_word(self.doc, apply_report_labels(d_disp, {'dimension_item': '版位'}), sub_title, level=2)
# 
#         # --- 7. 架构 ---
#         rows = []
#         if "Master_Breakdown" in self.merged_dfs:
...
#             add_df_to_word(self.doc, apply_report_labels(df_struct), "7. 广告架构简析", level=1)
# 
# 
# # ==========================================
# # PART 4: Streamlit UI
...
# 
#     st.info("请您上传[周期性复盘报告]、[行业benchmark]两个数据文件。本工具将为您输出三种文件，JSON格式可用于大模型分析，Excel可用于数据透视，Word格式可用于审查。建议：您可只选择下载JSON格式文件，如有必要再下载其他格式文件。")
# 
#     col1, col2 = st.columns(2)
#     with col1:
#         raw_file = st.file_uploader("1.请在此处上传 [周期性复盘报告]", type=["xlsx", "xls"])
#     with col2:
#         bench_file = st.file_uploader("2.请在此处上传 [行业Benchmark]", type=["xlsx", "xls"])
# 
#     if st.button("🚀 开始处理"):
#         if not raw_file:
#             st.error("请至少上传数据报表！")
...
# 1. 后台启动 Streamlit (确保 app.py 已存在)
